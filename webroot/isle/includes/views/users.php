...
<div class="listLeft center">
  <button type="button" name="addItemBtn" value="add" id="addItemBtn"
          class="btn btn-primary" autofocus="autofocus"><i class="icon-plus icon-white"></i> Add User</button>
</div>
...
<div class="modal hide" id="modalDialog" role="dialog" aria-labelledby="modalTitle">
  ...
  <form name="modalForm" id="modalForm" action="" method="post">
    ...
    <div class="modal-body">
          ...
          <option value="">-- Select a user</option>
            ...
            <?php
              ...
              foreach($employees as $employee) {
                ...
                echo '<option data-email="' . $employee['PRIMARYEMAIL'] .
                     '" value="' . $employee['EMPLOYEENUMBER'] . '"' .
                     $selected . '>' .
                     htmlspecialchars($employee['FULL_NAME']) . "</option>\n";
              }
            ?>
        </select>&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <input type="hidden" name="<?php echo $fieldNames['name']; ?>"
             id="<?php echo $fieldNames['name']; ?>" />
    
      <input type="hidden" name="<?php echo $fieldNames['email']; ?>"
             id="<?php echo $fieldNames['email']; ?>" />
    
      <div class="formItem newline">
        <label for="<?php echo $fieldNames['role']; ?>">Role</label>
        <select name="<?php echo $fieldNames['role']; ?>"
                id="<?php echo $fieldNames['role']; ?>">
          <option value="">-- Select a role</option>
          <!-- populate with data from database -->
          <?php
            foreach($roles as $role) {
              $selected = '';
              if(isset($_POST[$fieldNames['role']]) and
                 $_POST[$fieldNames['role']] == $role['id']) {
                $selected = ' selected="selected"';
              }
      ...
      <div class="formItem newline">
        <label for="chkEmail" id="labEmail"><input type="checkbox" name="chkEmail"
               value="" id="chkEmail" />Send a welcome email
          <span id="toEmail">&nbsp;</span>
          <span id="msg-chkEmail" class="err">&nbsp;</span>
        </label>
      </div>
    </div>
